...
export async function populateHeaderPane(headerEl: Element): Promise<void> {
    let userInfo: UserInfo | null = null;
    try {
        const { userInfo: extractedUserInfo } = await canConnectToBackend(this.settings.khojUrl, this.settings.khojApiKey, false);
